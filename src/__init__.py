...
def patchSource(hashS, ver):
    ...
    if ver > 27 and patchDump:
        ...
        replaceFileText('src/third_party/dart/runtime/vm/class_table.cc',
                        'OS::PrintErr("%" Pd ": %s\\n", i, name.ToCString());',
                        '\n     auto& funcs = Array::Handle(cls.functions());    if (funcs.Length()>1000) {    continue;    }	std::string classText=""; 	  String& supname = String::Handle();  	  name = cls.Name();	strcat(classText.c_str(),cls.ToCString());  	  Class& supcls = Class::Handle();    supcls = cls.SuperClass();  	  if (!supcls.IsNull()) {		 supname = supcls.Name();		  strcat(classText.c_str()," extends ");		 strcat(classText.c_str(),supname.ToCString()); 	}		  const auto& interfaces = Array::Handle(cls.interfaces());	auto& interface = Instance::Handle();		  for (intptr_t in = 0;in < interfaces.Length(); in++) {	interface^=interfaces.At(in);	if(in==0){strcat(classText.c_str()," implements ");} 	  if(in>0){strcat(classText.c_str()," , ");}		strcat(classText.c_str(),interface.ToCString());	}		  strcat(classText.c_str()," {\\n");	const auto& fields = Array::Handle(cls.fields());   	  auto& field = Field::Handle();	auto& fieldType = AbstractType::Handle(); 	  String& fieldTypeName = String::Handle();	String& finame = String::Handle();		  auto& instance2 = Instance::Handle();		  for (intptr_t f = 0; f < fields.Length(); f++)		  {    field ^= fields.At(f);	finame = field.name();	fieldType = field.type();	fieldTypeName = fieldType.Name();	strcat(classText.c_str(),"  ");		  strcat(classText.c_str(),fieldTypeName.ToCString()); 	strcat(classText.c_str()," ");	strcat(classText.c_str(),finame.ToCString()); 		  if(field.is_static()){			instance2 ^= field.StaticValue();			strcat(classText.c_str()," = ");			  strcat(classText.c_str(),instance2.ToCString());			strcat(classText.c_str()," ;\\n");  } 	  else {	  strcat(classText.c_str()," = ");	  strcat(classText.c_str()," nonstatic;\\n");  }	}  	  for (intptr_t c = 0; c < funcs.Length(); c++) {		    auto& func = Function::Handle();    func = cls.FunctionFromIndex(c);  	  String& signature = String::Handle();    signature = func.InternalSignature();auto& codee = Code::Handle(func.CurrentCode());	  	  if(!func.IsLocalFunction()) {		  strcat(classText.c_str()," \\n  ");	strcat(classText.c_str(),func.ToCString());	strcat(classText.c_str()," ");    strcat(classText.c_str(),signature.ToCString());		  strcat(classText.c_str()," { \\n\\n              ");	  char append[70];	  snprintf(append, sizeof(append), " Code Offset: _kDartIsolateSnapshotInstructions + 0x%016" PRIxPTR "\\n",static_cast<uintptr_t>(codee.MonomorphicUncheckedEntryPoint()));	  strcat(classText.c_str(),append);		  strcat(classText.c_str(),"       \\n       }\\n");		  } else {		  auto& parf = Function::Handle();	parf=func.parent_function();		  String& signParent = String::Handle();   		  signParent = parf.InternalSignature();			  strcat(classText.c_str()," \\n  ");			  strcat(classText.c_str(),parf.ToCString());	strcat(classText.c_str()," ");	strcat(classText.c_str(),signParent.ToCString());		  strcat(classText.c_str()," { \\n\\n          "); 	  char append[80];	  snprintf(append, sizeof(append), " Code Offset: _kDartIsolateSnapshotInstructions + 0x%016" PRIxPTR "\\n",static_cast<uintptr_t>(codee.MonomorphicUncheckedEntryPoint()));	  strcat(classText.c_str(),append);		  strcat(classText.c_str(),"       \\n       }\\n");		}	}		  	  strcat(classText.c_str()," \\n      }\\n\\n");	  	  const Library& libr = Library::Handle(cls.library());if (!libr.IsNull()) {  auto& owner_class = Class::Handle(); owner_class = libr.toplevel_class();   auto& funcsTopLevel = Array::Handle(owner_class.functions());   char pushTmp[1000];   String& owner_name = String::Handle();   owner_name = libr.url();   snprintf(pushTmp, sizeof(pushTmp), "\'%s\',",owner_name.ToCString());  if (funcsTopLevel.Length()>0&&strstr(pushArr.c_str(), pushTmp) == NULL) {  strcat(pushArr.c_str(),pushTmp);   strcat(classText.c_str(),"Library:"); strcat(classText.c_str(),pushTmp); strcat(classText.c_str()," {\\n");         for (intptr_t c = 0; c < funcsTopLevel.Length(); c++) {      auto& func = Function::Handle();    func = owner_class.FunctionFromIndex(c);  	  String& signature = String::Handle();    	  signature = func.InternalSignature();	  auto& codee = Code::Handle(func.CurrentCode());	   if(!func.IsLocalFunction()) {		  strcat(classText.c_str()," \\n  ");	strcat(classText.c_str(),func.ToCString());	strcat(classText.c_str()," ");    strcat(classText.c_str(),signature.ToCString());		  strcat(classText.c_str()," { \\n\\n              ");	  char append[70];	  snprintf(append, sizeof(append), " Code Offset: _kDartIsolateSnapshotInstructions + 0x%016" PRIxPTR "\\n",static_cast<uintptr_t>(codee.MonomorphicUncheckedEntryPoint()));	  strcat(classText.c_str(),append);		  strcat(classText.c_str(),"       \\n       }\\n");		  } else {		  auto& parf = Function::Handle();	parf=func.parent_function();		  String& signParent = String::Handle();   		  signParent = parf.InternalSignature();			  strcat(classText.c_str()," \\n  ");			  strcat(classText.c_str(),parf.ToCString());	strcat(classText.c_str()," ");	strcat(classText.c_str(),signParent.ToCString());		  strcat(classText.c_str()," { \\n\\n          "); 	  char append[80];	  snprintf(append, sizeof(append), " Code Offset: _kDartIsolateSnapshotInstructions + 0x%016" PRIxPTR "\\n",static_cast<uintptr_t>(codee.MonomorphicUncheckedEntryPoint()));	  strcat(classText.c_str(),append);		  strcat(classText.c_str(),"       \\n       }\\n");		}	  }             strcat(classText.c_str()," \\n      }\\n\\n");}}	  struct stat entry_info;	  int exists = 0;	  if (stat("/data/data/", &entry_info)==0 && S_ISDIR(entry_info.st_mode)){		  exists=1;	  }	  	  	  if(exists==1){		  pid_t pid = getpid();		  char path[64] = { 0 };		  snprintf(path, sizeof(path), "/proc/%d/cmdline", pid);		  		  FILE *cmdline = fopen(path, "r");		  if (cmdline) {			  	    char chm[264] = { 0 };		char pat[264] = { 0 };        char application_id[64] = { 0 };		        fread(application_id, sizeof(application_id), 1, cmdline);		snprintf(pat, sizeof(pat), "/data/data/%s/dump.dart", application_id);		        do { FILE *f = fopen(pat, "a+");   fprintf(f, "%s",classText.c_str());   fflush(f);   fclose(f);   snprintf(chm, sizeof(chm), "/data/data/%s",application_id);  chmod(chm, S_IRWXU|S_IRWXG|S_IRWXO);  chmod(pat, S_IRWXU|S_IRWXG|S_IRWXO);	  } while (0);        fclose(cmdline);    }	  }	  	  	  	  	  	  	  	  	  	  	  	  	  if(exists==0){			  	   		char pat[264] = { 0 };		snprintf(pat, sizeof(pat), "%s/Documents/dump.dart", getenv("HOME"));   OS::PrintErr("reFlutter dump file: %s",pat);     do { FILE *f = fopen(pat, "a+");   fprintf(f, "%s",classText.c_str());   fflush(f);   fclose(f);   	  } while (0);         	  }')
    elif ver < 28 and patchDump:
        replaceFileText('src/third_party/dart/runtime/vm/class_table.cc', '::Print() {',
                        '::Print()  { OS::PrintErr("reFlutter");\n std::string pushArr="";\n')
        replaceFileText('src/third_party/dart/runtime/vm/class_table.cc',
                        'OS::PrintErr("%" Pd ": %s\\n", i, name.ToCString());',
                        '\n      auto& funcs = Array::Handle(cls.functions());    if (funcs.Length()>1000) {    continue;    }	std::string classText=""; 	  String& supname = String::Handle();  	  name = cls.Name();	strcat(classText.c_str(),cls.ToCString());  	  Class& supcls = Class::Handle();    supcls = cls.SuperClass();  	  if (!supcls.IsNull()) {		 supname = supcls.Name();		  strcat(classText.c_str()," extends ");		 strcat(classText.c_str(),supname.ToCString()); 	}		  const auto& interfaces = Array::Handle(cls.interfaces());	auto& interface = Instance::Handle();		  for (intptr_t in = 0;in < interfaces.Length(); in++) {	interface^=interfaces.At(in);	if(in==0){strcat(classText.c_str()," implements ");} 	  if(in>0){strcat(classText.c_str()," , ");}		strcat(classText.c_str(),interface.ToCString());	}		  strcat(classText.c_str()," {\\n");	const auto& fields = Array::Handle(cls.fields());   	  auto& field = Field::Handle();	auto& fieldType = AbstractType::Handle(); 	  String& fieldTypeName = String::Handle();	String& finame = String::Handle();		  auto& instance2 = Instance::Handle();		  for (intptr_t f = 0; f < fields.Length(); f++)		  {    field ^= fields.At(f);	finame = field.name();	fieldType = field.type();	fieldTypeName = fieldType.Name();	strcat(classText.c_str(),"  ");		  strcat(classText.c_str(),fieldTypeName.ToCString()); 	strcat(classText.c_str()," ");	strcat(classText.c_str(),finame.ToCString()); 		  if(field.is_static()){			instance2 = field.StaticValue();			strcat(classText.c_str()," = ");			  strcat(classText.c_str(),instance2.ToCString());			strcat(classText.c_str()," ;\\n");  } 	  else {	  strcat(classText.c_str()," = ");	  strcat(classText.c_str()," nonstatic;\\n");  }	}  	  for (intptr_t c = 0; c < funcs.Length(); c++) {		    auto& func = Function::Handle();    func = cls.FunctionFromIndex(c);  	  String& signature = String::Handle();    signature = func.Signature();auto& codee = Code::Handle(func.CurrentCode());	  	  if(!func.IsLocalFunction()) {		  strcat(classText.c_str()," \\n  ");	strcat(classText.c_str(),func.ToCString());	strcat(classText.c_str()," ");    strcat(classText.c_str(),signature.ToCString());		  strcat(classText.c_str()," { \\n\\n              ");	  char append[70];	  snprintf(append, sizeof(append), " Code Offset: _kDartIsolateSnapshotInstructions + 0x%016" PRIxPTR "\\n",static_cast<uintptr_t>(codee.MonomorphicUncheckedEntryPoint()));	  strcat(classText.c_str(),append);		  strcat(classText.c_str(),"       \\n       }\\n");		  } else {		  auto& parf = Function::Handle();	parf=func.parent_function();		  String& signParent = String::Handle();   		  signParent = parf.Signature();			  strcat(classText.c_str()," \\n  ");			  strcat(classText.c_str(),parf.ToCString());	strcat(classText.c_str()," ");	strcat(classText.c_str(),signParent.ToCString());		  strcat(classText.c_str()," { \\n\\n          "); 	  char append[80];	  snprintf(append, sizeof(append), " Code Offset: _kDartIsolateSnapshotInstructions + 0x%016" PRIxPTR "\\n",static_cast<uintptr_t>(codee.MonomorphicUncheckedEntryPoint()));	  strcat(classText.c_str(),append);		  strcat(classText.c_str(),"       \\n       }\\n");		}	}		  	  strcat(classText.c_str()," \\n      }\\n\\n");	  	  const Library& libr = Library::Handle(cls.library());if (!libr.IsNull()) {  auto& owner_class = Class::Handle(); owner_class = libr.toplevel_class();   auto& funcsTopLevel = Array::Handle(owner_class.functions());   char pushTmp[1000];   String& owner_name = String::Handle();   owner_name = libr.url();   snprintf(pushTmp, sizeof(pushTmp), "\'%s\',",owner_name.ToCString());  if (funcsTopLevel.Length()>0&&strstr(pushArr.c_str(), pushTmp) == NULL) {  strcat(pushArr.c_str(),pushTmp);   strcat(classText.c_str(),"Library:"); strcat(classText.c_str(),pushTmp); strcat(classText.c_str()," {\\n");         for (intptr_t c = 0; c < funcsTopLevel.Length(); c++) {      auto& func = Function::Handle();    func = owner_class.FunctionFromIndex(c);  	  String& signature = String::Handle();    	  signature = func.Signature();	  auto& codee = Code::Handle(func.CurrentCode());	   if(!func.IsLocalFunction()) {		  strcat(classText.c_str()," \\n  ");	strcat(classText.c_str(),func.ToCString());	strcat(classText.c_str()," ");    strcat(classText.c_str(),signature.ToCString());		  strcat(classText.c_str()," { \\n\\n              ");	  char append[70];	  snprintf(append, sizeof(append), " Code Offset: _kDartIsolateSnapshotInstructions + 0x%016" PRIxPTR "\\n",static_cast<uintptr_t>(codee.MonomorphicUncheckedEntryPoint()));	  strcat(classText.c_str(),append);		  strcat(classText.c_str(),"       \\n       }\\n");		  } else {		  auto& parf = Function::Handle();	parf=func.parent_function();		  String& signParent = String::Handle();   		  signParent = parf.Signature();			  strcat(classText.c_str()," \\n  ");			  strcat(classText.c_str(),parf.ToCString());	strcat(classText.c_str()," ");	strcat(classText.c_str(),signParent.ToCString());		  strcat(classText.c_str()," { \\n\\n          "); 	  char append[80];	  snprintf(append, sizeof(append), " Code Offset: _kDartIsolateSnapshotInstructions + 0x%016" PRIxPTR "\\n",static_cast<uintptr_t>(codee.MonomorphicUncheckedEntryPoint()));	  strcat(classText.c_str(),append);		  strcat(classText.c_str(),"       \\n       }\\n");		}	  }             strcat(classText.c_str()," \\n      }\\n\\n");}}	  struct stat entry_info;	  int exists = 0;	  if (stat("/data/data/", &entry_info)==0 && S_ISDIR(entry_info.st_mode)){		  exists=1;	  }	  	  	  if(exists==1){		  pid_t pid = getpid();		  char path[64] = { 0 };		  snprintf(path, sizeof(path), "/proc/%d/cmdline", pid);		  		  FILE *cmdline = fopen(path, "r");		  if (cmdline) {			  	    char chm[264] = { 0 };		char pat[264] = { 0 };        char application_id[64] = { 0 };		        fread(application_id, sizeof(application_id), 1, cmdline);		snprintf(pat, sizeof(pat), "/data/data/%s/dump.dart", application_id);		        do { FILE *f = fopen(pat, "a+");   fprintf(f, "%s",classText.c_str());   fflush(f);   fclose(f);   snprintf(chm, sizeof(chm), "/data/data/%s",application_id);  chmod(chm, S_IRWXU|S_IRWXG|S_IRWXO);  chmod(pat, S_IRWXU|S_IRWXG|S_IRWXO);	  } while (0);        fclose(cmdline);    }	  }	  	  	  	  	  	  	  	  	  	  	  	  	  if(exists==0){			  	   		char pat[264] = { 0 };        snprintf(pat, sizeof(pat), "%s/Documents/dump.dart", getenv("HOME"));	OS::PrintErr("reFlutter dump file: %s",pat); 	        do { FILE *f = fopen(pat, "a+");   fprintf(f, "%s",classText.c_str());   fflush(f);   fclose(f);   	  } while (0);         	  }')
