...
def replaceLibFlutter():
    ...
    if (
        ...
        try:
            shutil.move(
                "libflutter_arm64.so",
                join("release", libAppArm64[0].replace("libapp.so", "libflutter.so")),
            )
        except:
            pass
        try:
            shutil.move(
                "libflutter_arm.so",
                join("release", libAppArm[0].replace("libapp.so", "libflutter.so")),
            )
        except:
            pass
        try:
            shutil.move(
                "libflutter_x64.so",
                join("release", libAppX64[0].replace("libapp.so", "libflutter.so")),
            )
        except:
            pass
        try:
...
def extractZip(zipname):
    ...
    with ZipFile(zipname, "r") as zipObject:
        ...
        for fileName in listOfFileNames:
            if fileName.endswith("App.framework/App") or fileName.endswith(
...
def patch_ssl_x509():
    try:
        ...
        with open(f"src/flutter/third_party/boringssl/src/ssl/{SSL_X590}", "w") as f:
